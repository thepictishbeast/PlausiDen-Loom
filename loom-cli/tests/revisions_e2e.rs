...
fn revisions_list_orders_newest_first_with_human_timestamps() {
    ...
    write(
        &cms.join("home.json"),
        r#"{"title":"active","sections":[]}"#,
    );
    // Synthetic backups with three different unix timestamps.
    write(
        &cms.join("home.bak.1700000000.000000001.json"),
        r#"{"title":"oldest"}"#,
    );
    write(
        &cms.join("home.bak.1700000050.000000002.json"),
        r#"{"title":"middle"}"#,
    );
    write(
        &cms.join("home.bak.1700000100.000000003.json"),
        r#"{"title":"newest"}"#,
    );
...
fn revisions_show_prints_content_of_indexed_revision() {
    ...
    write(
        &cms.join("home.bak.1700000100.0.json"),
        r#"{"title":"NEWER"}"#,
    );
    write(
        &cms.join("home.bak.1700000050.0.json"),
        r#"{"title":"OLDER"}"#,
    );
    ...
    assert_eq!(status, 0);
    assert!(
        out.contains("\"title\":\"NEWER\""),
        "rev 1 should be NEWER:\n{out}"
    );
    // Index 2 = older.
    let (_, out2, _) = run(&cms, &["show", "home", "2"]);
    assert!(
        out2.contains("\"title\":\"OLDER\""),
        "rev 2 should be OLDER:\n{out2}"
    );
...
fn revisions_diff_shows_minus_revision_plus_active() {
    ...
    assert_eq!(status, 0);
    assert!(
        out.contains("--- ") && out.contains("(revision 1)"),
        "expected unified-diff header:\n{out}"
    );
    assert!(
        out.contains("+++ ") && out.contains("(active)"),
        "expected unified-diff +++ line:\n{out}"
    );
    assert!(
        out.contains("-rev line A"),
        "expected `-rev line A`:\n{out}"
    );
    assert!(
        out.contains("+active line A"),
        "expected `+active line A`:\n{out}"
    );
    // shared line MUST NOT appear (it's in both).
    assert!(
        !out.lines()
            .any(|l| l == "-shared line" || l == "+shared line"),
...
fn revisions_restore_replaces_active_and_snapshots_prior() {
    ...
    assert!(
        out.contains("restored from revision 1"),
...
fn revisions_list_all_slugs_aggregates_across_pages() {
    ...
    write(
        &cms.join("about.bak.1700000200.0.json"),
        "about rev (middle)",
    );
    ...
    assert!(
        stdout.contains("home.bak.1700000300"),
        "newest home rev missing:\n{stdout}"
    );
    assert!(
        stdout.contains("about.bak.1700000200"),
        "about rev missing:\n{stdout}"
    );
    assert!(
        stdout.contains("home.bak.1700000100"),
    ...
    assert!(
        idx_300 < idx_200 && idx_200 < idx_100,
...
fn revisions_list_all_slugs_lines_caps_output() {
    ...
    assert!(
        stdout.contains("1700000007"),
        "newest must appear:\n{stdout}"
    );
    assert!(
        stdout.contains("showing 3 of 8"),
...
fn revisions_list_empty_slug_prints_friendly_message() {
    ...
    assert!(
        out.contains("no backups for 'home'"),
